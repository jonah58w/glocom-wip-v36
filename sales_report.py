...
def render_sales_report_page(
    ...
    **kwargs,
    ...
    with st.expander("欄位偵測"):
        ...
        tooling_col = e2.selectbox("TOOLING欄位", all_cols, index=idx_for(tooling_auto))
        hold_col = e3.selectbox("HOLD欄位", all_cols, index=idx_for(hold_auto))
        discount_col = e4.selectbox("折讓欄位", all_cols, index=idx_for(discount_auto))
    ...
    if company_name.strip() and customer_col and customer_col in month_df.columns:
        month_df = month_df[
            month_df[customer_col].astype(str).str.strip().str.upper()
